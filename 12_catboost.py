...
OUT_DIR    = "output_catboost_no_hform_thick"
...
def main():
    ...
    for T in targets:
        ...
        print(f"\n计算 Pearson 并选择 Top-13 特征 —— 目标: {tgt}")
        ...
        corr_df.head(13).to_csv(os.path.join(OUT_DIR, f"{tgt}_pearson_top13.csv"), index=False)
        print(corr_df.head(13))

        plot_pearson_matrix(
            X_all[top_feats], y_raw,
        ...
        for fold, (tr, te) in enumerate(gkf.split(X, y_trainable, groups)):
            ...
            base = CatBoostRegressor(verbose=False, random_seed=42, eval_metric="R2")
            ...
            search.fit(X_tr_in, y_tr_in, **fit_params)
            ...
            if fold == 0:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(OUT_DIR, f"{tgt}_shap_top13.png"), dpi=600)
                plt.close()

        mean_r2, mean_mae, mean_rmse = np.mean(fold_metrics, axis=0)
        print(f"\n{tgt} 平均性能: R²={mean_r2:.3f}, MAE={mean_mae:.3f}, RMSE={mean_rmse:.3f}")

        plt.figure(figsize=(6.2, 6))
        plt.scatter(y_true_all, y_pred_all, alpha=0.6, color="#2aa198", edgecolor="none")
        lims = [min(y_true_all + y_pred_all), max(y_true_all + y_pred_all)]
        plt.plot(lims, lims, "r--", lw=1)
        plt.xlabel(f"True {tgt} (eV)")
        plt.ylabel(f"Predicted {tgt} (eV)")
        plt.title(f"CatBoost — {tgt}")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_DIR, f"{tgt}_pred_vs_true.png"), dpi=600)
        plt.close()

        pd.DataFrame([{
            "target": tgt, "R2": mean_r2, "MAE": mean_mae, "RMSE": mean_rmse,
            "n_samples": len(dft)
        }]).to_csv(os.path.join(OUT_DIR, f"{tgt}_metrics.csv"), index=False)
